...
## Deprecated Windows task scripts

Top-level `task/` không còn là active project path. Các PowerShell helper cũ đã được chuyển sang `deprecated/task/`:

- `deprecated/task/windows.smss.ps1`
- `deprecated/task/windows.ssh.ps1`
- `deprecated/task/windows.time.ps1`

Các file này chỉ giữ làm tham khảo lịch sử, không dùng làm runtime path chính.
