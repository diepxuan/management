...
*
*/
class hyperdb_mysql extends wpdb {

  /**
   * Valid mysql_query
   */
  function _mysql_query( $query, $dbh ) {
    $_return = FALSE;
    if ( function_exists( 'mysql_query' ) ) {
      $_return = mysql_query( $query, $dbh );
    } elseif ( function_exists( 'mysqli_query' ) ) {
      try {
        $_return = mysqli_query( $dbh, $query );
  ...
  /**
   * Valid mysql_set_charset
   */
  function _mysql_set_charset( $charset, $dbh ) {
    $_return = FALSE;
    if ( function_exists( 'mysql_set_charset' ) ) {
      $_return = mysql_set_charset( $charset, $dbh );
    } elseif ( function_exists( 'mysqli_set_charset' ) ) {
      try {
        $_return = mysqli_set_charset( $dbh, $charset );
      } catch (mysqli_sql_exception $e) {
        throw $e;
      }
      return $_return;
   ...
   * Valid mysql_close
   */
  function _mysql_close( $dbh ) {
    $_return = FALSE;
    if ( function_exists( 'mysql_close' ) ) {
      $_return = mysql_close( $dbh );
    } elseif ( function_exists( 'mysqli_close' ) ) {
      try {
        $_return = mysqli_close( $dbh );
      } catch (mysqli_sql_exception $e) {
        throw $e;
      }
    }
   ...
   * Valid mysql_error
   */
  function _mysql_error( $dbh=null ) {
    $_return = FALSE;
    if ( function_exists( 'mysql_error' ) ) {
      if( $dbh ) {
        $_return = mysql_error( $dbh );
      } else {
        $_return = mysql_error();
      }
    } elseif ( function_exists( 'mysqli_error' ) ) {
      if( $dbh ) {
        $_return = mysqli_error( $dbh );
      } else {
        $_return = mysqli_connect_error();
      }
    }
    return $_return;
  }
  ...
  /**
   * Valid mysql_affected_rows
   */
  function _mysql_affected_rows( $dbh ) {
    $_return = FALSE;
    if ( function_exists( 'mysql_affected_rows' ) ) {
      $_return = mysql_affected_rows( $dbh );
    } elseif ( function_exists( 'mysqli_affected_rows' ) ) {
      try {
        $_return = mysqli_affected_rows( $dbh );
      } catch (mysqli_sql_exception $e) {
        throw $e;
      }
    }
  ...
  /**
   * Valid mysql_insert_id
   */
  function _mysql_insert_id( $dbh ) {
    $_return = FALSE;
    if ( function_exists( 'mysql_insert_id' ) ) {
      $_return = mysql_insert_id( $dbh );
    } elseif ( function_exists( 'mysqli_insert_id' ) ) {
      try {
        $_return = mysqli_insert_id( $dbh );
      } catch (mysqli_sql_exception $e) {
        throw $e;
      }
    }
  ...
  /**
   * Valid mysql_num_fields
   */
  function _mysql_num_fields( $result ) {
    $_return = FALSE;
    if ( function_exists( 'mysql_num_fields' ) ) {
      $_return = mysql_num_fields( $result );
    } elseif ( function_exists( 'mysqli_num_fields' ) ) {
      try {
        $_return = mysqli_num_fields( $result );
      } catch (mysqli_sql_exception $e) {
        throw $e;
      }
    }
  ...
  /**
   * Valid mysql_fetch_field
   */
  function _mysql_fetch_field( $result ) {
    $_return = FALSE;
    if ( function_exists( 'mysql_fetch_field' ) ) {
      $_return = mysql_fetch_field( $result );
    } elseif ( function_exists( 'mysqli_fetch_field' ) ) {
      try {
        $_return = mysqli_fetch_field( $result );
      } catch (mysqli_sql_exception $e) {
        throw $e;
      }
    }
  ...
  /**
   * Valid mysql_fetch_object
   */
  function _mysql_fetch_object( $result ) {
    $_return = FALSE;
    if ( function_exists( 'mysql_fetch_object' ) ) {
      $_return = mysql_fetch_object( $result );
    } elseif ( function_exists( 'mysqli_fetch_object' ) ) {
      try {
        $_return = mysqli_fetch_object( $result );
      } catch (mysqli_sql_exception $e) {
        throw $e;
      }
    }
  ...
  /**
   * Valid mysql_get_server_info
   */
  function _mysql_get_server_info( $dbh ) {
    $_return = FALSE;
    if ( function_exists( 'mysql_get_server_info' ) ) {
      $_return = mysql_get_server_info( $dbh );
    } elseif ( function_exists( 'mysqli_get_server_info' ) ) {
      try {
        $_return = mysqli_get_server_info( $dbh );
      } catch (mysqli_sql_exception $e) {
        throw $e;
      }
    }
  ...
  /**
   * Valid mysql_select_db
   */
  function _mysql_select_db( $dbname, $dbh ) {
    $_return = FALSE;
    if ( function_exists( 'mysql_select_db' ) ) {
      $_return = mysql_select_db( $dbname, $dbh );
    } elseif ( function_exists( 'mysqli_select_db' ) ) {
      try {
        $_return = mysqli_select_db( $dbh, $dbname );
      } catch (mysqli_sql_exception $e) {
        throw $e;
      }
    }
   ...
   * Valid mysql_connect
   */
  function _mysql_connect($DB_HOST, $DB_USER, $DB_PASSWORD, $new_link = true) {
    $_return = FALSE;
    ...
    if ( function_exists('mysql_connect') ) {
      ...
      $_return = @ $connect_function( $DB_HOST, $DB_USER, $DB_PASSWORD, $new_link ) or die( $this->_mysql_error() );
      ...
      try {
        ...
        $_return = mysqli_connect( $persist.$DB_HOST, $DB_USER, $DB_PASSWORD, '', $DB_PORT ) or die( $this->_mysql_error() );
      } catch (mysqli_sql_exception $e) {
        throw $e;
      }
    }
   ...
   * Valid mysql_ping
   */
  function _mysql_ping( $dbh ) {
    $_return = FALSE;
    if ( function_exists( 'mysql_ping' ) ) {
      $_return = mysql_ping( $dbh );
    } elseif ( function_exists( 'mysqli_ping' ) ) {
      try {
        $_return = mysqli_ping( $dbh );
      } catch (mysqli_sql_exception $e) {
        throw $e;
      }
    }
   ...
   * Valid $dbh
   */
  function _is_resource( $dbh ) {
    $_return = FALSE;
    $_return = $_return || is_resource($dbh);
    $_return = $_return || $dbh instanceof mysqli;
    return $_return;
  }
...
class hyperdb extends hyperdb_mysql {
  /**
  ...
  function db_connect( $query = '' ) {
    ...
    if ( empty( $this->hyper_servers ) ) {
      ...
      $this->dbh = $this->_mysql_connect(DB_HOST, DB_USER, DB_PASSWORD, true);
      if ( ! $this->_is_resource( $this->dbh ) )
        return $this->bail("We were unable to connect to the database. (DB_HOST)");
      if ( ! $this->_mysql_select_db(DB_NAME, $this->dbh) )
        return $this->bail("We were unable to select the database. (DB_NAME)");
    ...
    while ( isset( $this->dbhs[$dbhname] ) && $this->_is_resource( $this->dbhs[$dbhname] ) ) {
      ...
      if ( isset($server['name']) ) {
        ...
        if ( $name != $this->used_servers[$dbhname]['name'] ) {
          if ( !$this->_mysql_select_db($name, $this->dbhs[$dbhname]) ) {
      ...
      if ( !$this->_mysql_ping( $this->dbhs[$dbhname] ) ) {
    ...
    do {
      ...
      foreach ( $servers as $group_key ) {
        ...
        {
          $this->dbhs[$dbhname] = $this->_mysql_connect( "$host:$port", $user, $password, true );
        } else {
        ...
        if ( $this->_is_resource( $this->dbhs[$dbhname] ) ) {
          /**
          ...
          if ( !$use_master && !$write && !isset( $ignore_slave_lag )
            ...
            continue;
          } elseif ( $this->_mysql_select_db( $name, $this->dbhs[ $dbhname ] ) ) {
            $success = true;
        ...
        // $msg .= "'server' => {$server},\n";
        $msg .= "'host' => {$host},\n";
        $msg .= "'error' => " . $this->_mysql_error() . ",\n";
        $msg .= "'errno' => " . $this->_mysql_error() . ",\n";
      ...
      if ( !$success || !isset($this->dbhs[$dbhname]) || !$this->_is_resource( $this->dbhs[$dbhname] ) ) {
  ...
  function set_charset($dbh, $charset = null, $collate = null) {
    ...
    if ( $this->has_cap( 'collation', $dbh ) && !empty( $charset ) ) {
      if ( (function_exists( 'mysql_set_charset' ) || function_exists( 'mysqli_set_charset' ) ) && $this->has_cap( 'set_charset', $dbh ) ) {
        $this->_mysql_set_charset( $charset, $dbh );
        $this->real_escape = true;
      } else {
        $query = $this->prepare( 'SET NAMES %s', $charset );
        if ( ! empty( $collate ) )
          $query .= $this->prepare( ' COLLATE %s', $collate );
        $this->_mysql_query( $query, $dbh );
      }
    }
  }
  ...
  function disconnect($dbhname) {
    ...
    if ( $this->_is_resource($this->dbhs[$dbhname]) )
      $this->_mysql_close($this->dbhs[$dbhname]);
  ...
  function query($query) {
    ...
    if ( preg_match('/^\s*SELECT\s+FOUND_ROWS(\s*)/i', $query) && $this->_is_resource($this->last_found_rows_result) ) {
      ...
      $this->timer_start();
      $this->result = $this->_mysql_query($query, $this->dbh);
      $elapsed = $this->timer_stop();
      ++$this->num_queries;
      ...
      if ( preg_match('/^\s*SELECT\s+SQL_CALC_FOUND_ROWS\s/i', $query) ) {
        if ( false === strpos($query, "NO_SELECT_FOUND_ROWS") ) {
          $this->timer_start();
          $this->last_found_rows_result = $this->_mysql_query("SELECT FOUND_ROWS()", $this->dbh);
          $elapsed += $this->timer_stop();
          ++$this->num_queries;
    ...
    if ( $this->last_error = $this->_mysql_error($this->dbh) ) {
      $this->print_error($this->last_error);
      return false;
    }

    if ( preg_match("/^\\s*(insert|delete|update|replace|alter) /i",$query) ) {
      $this->rows_affected = $this->_mysql_affected_rows($this->dbh);

      // Take note of the insert_id
      if ( preg_match("/^\\s*(insert|replace) /i",$query) ) {
        $this->insert_id = $this->_mysql_insert_id($this->dbh);
      }
      ...
      $i = 0;
      $this->col_info = array();
      while ($i < $this->_mysql_num_fields($this->result)) {
        $this->col_info[$i] = $this->_mysql_fetch_field($this->result);
        $i++;
      }
      $num_rows = 0;
      $this->last_result = array();
      while ( $row = $this->_mysql_fetch_object($this->result) ) {
        $this->last_result[$num_rows] = $row;
        $num_rows++;
      }

      $this->_mysql_free_result($this->result);
  ...
  function db_version( $dbh_or_table = false ) {
    if ( !$dbh_or_table && $this->dbh )
      $dbh =& $this->dbh;
    elseif ( $this->_is_resource( $dbh_or_table ) )
      $dbh =& $dbh_or_table;
    else
      $dbh = $this->db_connect( "SELECT FROM $dbh_or_table $this->users" );

    if ( $dbh )
      return preg_replace('/[^0-9.].*/', '', $this->_mysql_get_server_info( $dbh ));
    return false;
  }
